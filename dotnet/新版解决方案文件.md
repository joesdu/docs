...
> 前几天.NET9 Preview3 更新了,同时更新了 VS 以及一些工具和 Nuget 包.

今天我们讨论的内容则是 VS2022 17.10.0 Preview 3.0 带来的新的解决方案文件.

旧的解决方案文件的后缀为 **.sln** 这里我用我们的 EasilyNET 项目的来做个例子.
...
<Solution>
  ...
  <Folder Name="/src/Mongo/">
    ...
    <Project Path="src\EasilyNET.MongoDistributedLock.AspNetCore\EasilyNET.MongoDistributedLock.AspNetCore.csproj" />
    <Project Path="src\EasilyNET.MongoDistributedLock\EasilyNET.MongoDistributedLock.csproj" />
    <Project Path="src\EasilyNET.MongoGridFS.AspNetCore\EasilyNET.MongoGridFS.AspNetCore.csproj" />
    <Project Path="src\EasilyNET.MongoSerializer.AspNetCore\EasilyNET.MongoSerializer.AspNetCore.csproj" />
  </Folder>
  <Folder Name="/src/Framework/">
    ...
    <Project Path="src\EasilyNET.Images\EasilyNET.Images.csproj" />
...
接下来我们便可以通过新的解决方案文件进行项目管理.不过目前还存在一些问题,比如 Rider 和 Reshaper 可能还无法识别新的格式,所以我们还是需要保留旧的 **.sln** .同样在 GitHub Actions 里面也还无法识别,需要等待后面的更新,不过我们可以看到一切都在向更好的方向发展.
